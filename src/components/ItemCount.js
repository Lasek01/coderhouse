...
const ItemCount = ({stock, addToCart}) => {
    ...
    <div className="flex border-t border-b mb-6 border-gray-200 py-2">
                            ...
                            <div className="flex">
                            <span className="text-gray-500 text-lg pt-1">Stock disponible: {stock}</span><button className="flex ml-auto text-white bg-teal-500 border-0 py-2 px-6 focus:outline-none hover:bg-teal-600 rounded" onClick={()=>addToCart(count)}>Agregar al carrito &nbsp; <h/> <CartWidget/></button>
                            </div>
    </>
    )
}
